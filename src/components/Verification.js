...
const Verification = () => {
    ...
    const onClick=(e)=>{
      ...
      if(otp===realotp.otp){
        newUser(UserD);
        showAlert("Success: ","Your Account has been created","green")
        hist("/login");
      }
      else{
      showAlert("Error ","Enter the correct OTP","red")
      }
    }
    const SendOTPFunc=async()=>{
      OTPForm.current.classList.remove("hidden")
      let op = await getOtp(UserD.Email)
      setreaotp(op)
        ...
        <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
          ...
          <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <p>
                We will be sending an email to Email Address: {UserD.Email} <br/>to verify your account
              </p>
