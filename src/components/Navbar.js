  ...
  <div className='mb-3 top-0'>
    <div className='w-full fixed shadow-gray-300 shadow-md z-50'>
        ...
        <div className="container flex flex-wrap items-center justify-between mx-auto ">
          ...
          <div className="hidden w-full md:block md:w-auto" id="navbar-default" ref={navbar}>
            <ul className="flex flex-col p-4 mt-4 border md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 dark:bg-gray-400 md:dark:bg-gray-400 dark:border-gray-700">
              <li>
                <Link to="/" className=" block py-2 pl-3 pr-4 dark:text-gray-700 bg-white rounded md:bg-transparent md:p-0 md:hover:text-blue-700 " aria-current="page" onClick={()=>{props.resetPage()}}><span className='flex flex-row'><AiFillHome className='text-2xl'/><span className='md:hidden'>Home</span></span></Link>
              </li>
              <li>
                <Link to="/create" className="block py-2 pl-3 pr-4 rounded hover:bg-orange-500 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-100 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-orange-500 " onClick={()=>{props.resetPage()}}><span className='flex flex-row'><IoMdCreate className='text-2xl'/><span className='md:hidden'>Create</span></span></Link>
              </li>
              <li>
                <Link to={`/profile/${localStorage.getItem('token')}`} className="block py-2 pl-3 pr-4  rounded hover:bg-orange-500 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-100 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-orange-500 " onClick={()=>{props.resetPage()}}><span className='flex flex-row'><IoMdContact className='text-3xl'/><span className='md:hidden'>Profile</span></span></Link>
              </li>
            </ul>
          </div>
        </div>
